...
import expo.modules.innertube.models.Context
import expo.modules.innertube.models.MediaInfo
import expo.modules.innertube.models.ReturnYouTubeDislikeResponse
import expo.modules.innertube.models.YouTubeClient
import expo.modules.innertube.models.YouTubeLocale
import expo.modules.innertube.models.body.*
import expo.modules.innertube.models.response.NextResponse
import expo.modules.innertube.utils.parseCookieString
import expo.modules.innertube.utils.sha1
...
class InnerTube {
          ...
          proxyAuth?.let {
            config {
              proxyAuthenticator { _, response ->
                response.request.newBuilder().header("Proxy-Authorization", proxyAuth!!).build()
              }
            }
          }
        }
  ...
  suspend fun search(
    ...
    httpClient.post("search") {
      ...
      setBody(
        SearchBody(
          context =
            client.toContext(locale, visitorData, if (useLoginForBrowse) dataSyncId else null),
          query = query,
          params = params,
        )
      )
  ...
  suspend fun feedback(client: YouTubeClient, tokens: List<String>) =
  ...
  suspend fun getSearchSuggestions(client: YouTubeClient, input: String) =
  ...
  suspend fun getQueue(client: YouTubeClient, videoIds: List<String>?, playlistId: String?) =
  ...
  suspend fun getTranscript(client: YouTubeClient, videoId: String) =
  ...
  suspend fun likeVideo(client: YouTubeClient, videoId: String) =
  ...
  suspend fun unlikeVideo(client: YouTubeClient, videoId: String) =
  ...
  suspend fun subscribeChannel(client: YouTubeClient, channelId: String) =
  ...
  suspend fun unsubscribeChannel(client: YouTubeClient, channelId: String) =
  ...
  suspend fun likePlaylist(client: YouTubeClient, playlistId: String) =
    httpClient.post("like/like") {
      ytClient(client, setLogin = true)
      setBody(
        LikeBody(
  ...
  suspend fun unlikePlaylist(client: YouTubeClient, playlistId: String) =
    httpClient.post("like/removelike") {
      ytClient(client, setLogin = true)
      setBody(
        LikeBody(
  ...
  suspend fun addToPlaylist(client: YouTubeClient, playlistId: String, videoId: String) =
    ...
    httpClient.post("browse/edit_playlist") {
      ...
      setBody(
        EditPlaylistBody(
          ...
          playlistId = playlistId.removePrefix("VL"),
          actions =
            listOf(Action.RemoveVideoAction(removedVideoId = videoId, setVideoId = setVideoId)),
        )
      )
    }
  ...
  suspend fun createPlaylist(client: YouTubeClient, title: String) =
  ...
  suspend fun renamePlaylist(client: YouTubeClient, playlistId: String, name: String) =
    httpClient.post("browse/edit_playlist") {
      ytClient(client, setLogin = true)
      setBody(
        EditPlaylistBody(
  ...
  suspend fun getUploadCustomThumbnailLink(client: YouTubeClient, contentLength: Int) =
  ...
  suspend fun uploadCustomThumbnail(client: YouTubeClient, uploadId: String, image: ByteArray) =
    httpClient.post("https://music.youtube.com/playlist_image_upload/playlist_custom_thumbnail") {
      ytClient(client, setLogin = true)
  ...
  suspend fun setThumbnailPlaylist(client: YouTubeClient, playlistId: String, blobId: String) =
    httpClient.post("browse/edit_playlist") {
      ytClient(client, setLogin = true)
      setBody(
        EditPlaylistBody(
  ...
  suspend fun removeThumbnailPlaylist(client: YouTubeClient, playlistId: String) =
    httpClient.post("browse/edit_playlist") {
      ytClient(client, setLogin = true)
      setBody(
        EditPlaylistBody(
  ...
  suspend fun deletePlaylist(client: YouTubeClient, playlistId: String) =
